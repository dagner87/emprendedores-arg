...
class Ion_auth_model extends CI_Model
{
	...
	public function forgotten_password($identity)
	{
		...
		if ($return){
			...
			//preparae el email de recuperacion
			$cuerpo_mensaje = 'Por favor vaya a esta url para resetear su password:  
							</br> URL: <a href="'.base_url().'auth/reset_password/'.$key.'">'.base_url().'auth/reset_password/'.$key.'</a>';	

					
						
					//print_r($cuerpo_mensaje);die();			
			$this->sendMailGmail($identity,$cuerpo_mensaje);		
	...
	public function register_rev( $id_usuario,$identity, $password, $email, $activation_code,$first_name, $last_name, $company, $phone)
	{
		...
		$salt       = $this->store_salt ? $this->salt() : FALSE;
		$password   = $this->hash_password($password, $salt);
		$texto_consulta = "UPDATE  usuarios SET active=0, password='$password', username='$identity', first_name='$first_name',last_name='$last_name',company='$company',phone ='$phone', activation_code='$activation_code'   WHERE (id = $id_usuario);"; 
	...
	**/
	...
	public function obt_id_user($email)
	{
	...
	public function registrar_subordinado($usuario, $subordinado)
	{
		
		$texto_consulta =  "INSERT IGNORE INTO usuario_subordinados (id_usuario,id_subordinado)" 
		                  . " VALUES($usuario, $subordinado);";
	...
	public function obt_subordinados($usuario)
	{
		$texto_consulta = "SELECT   `usuarios`.*,  `usuario_subordinados`.`id_subordinado`
							FROM  `usuarios`
  							INNER JOIN `usuario_subordinados` ON (`usuarios`.`id` = `usuario_subordinados`.`id_subordinado`)
		 				 	WHERE `usuario_subordinados`.`id_usuario`=$usuario";
	...
	{
		$texto_consulta = "SELECT  id_usuario
							FROM  `usuario_subordinados` where id_subordinado = $id_usuario;";
	...
	public function act_superior($id_usuario, $id_superior)
	{
		$texto_consulta = "UPDATE usuario_subordinados SET id_usuario = $id_superior WHERE (id_subordinado=$id_usuario);";
